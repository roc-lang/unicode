interface CodePt
    exposes [Utf8ParseErr, CodePt, utf8Len, toU32, fromU32, isHighSurrogate, isLowSurrogate, isValidScalar, appendToUtf8, countUtf8Bytes]
...
parseUtf8 : List U8 -> Result { codePt : CodePt, bytesParsed : Nat } Utf8ParseErr
parseUtf8 = \bytes ->
...
expect [0xC3, 0x28] |> parseUtf8 == Err ExpectedContinuation

## Overlong encoding for ASCII character
expect [0xC0, 0xA1] |> parseUtf8 == Err OverlongEncoding

## Overlong encoding for 2-byte character
expect [0xE0, 0x80, 0xAF] |> parseUtf8 == Err OverlongEncoding

## Overlong encoding for 3-byte character
expect [0xF0, 0x80, 0x80, 0xA6] |> parseUtf8 == Err OverlongEncoding

## Invalid four-byte encoding (scalar value too large)
expect [0xF4, 0x90, 0x80, 0x80] |> parseUtf8 == Err CodepointTooLarge

## Invalid first byte (>= 0xF8)
expect [0xF8, 0xA1, 0xA2, 0xA3, 0xA4] |> parseUtf8 == Err InvalidUtf8

## Invalid first byte (>= 0xC0 and <= 0xC1)
expect [0xC0, 0x80] |> parseUtf8 == Err OverlongEncoding
...
expect "𠜎a" |> Str.toUtf8 |> parseUtf8 == Ok { codePt: fromU32Unchecked '𠜎', bytesParsed: 4 }

## Valid 4-byte followed by valid multi-byte scalar
expect "𠜎爱" |> Str.toUtf8 |> parseUtf8 == Ok { codePt: fromU32Unchecked '𠜎', bytesParsed: 4 }
...
## Valid 2-byte followed by multiple single-byte scalar
expect (List.concat (Str.toUtf8 "é") [0xC0, 0x80]) |> parseUtf8 == Ok { codePt: fromU32Unchecked 'é', bytesParsed: 2 }
...
expect (List.concat (Str.toUtf8 "𠜎") [0xC0, 0x80]) |> parseUtf8 == Ok { codePt: fromU32Unchecked '𠜎', bytesParsed: 4 }

## Invalid byte within a valid 2-byte character
expect [0xC3, 0xFF] |> parseUtf8 == Err ExpectedContinuation

## Invalid byte within a valid 3-byte character
expect [0xE2, 0x82, 0xFF] |> parseUtf8 == Err ExpectedContinuation

## Invalid byte within a valid 4-byte character
expect [0xF0, 0x9F, 0x98, 0xFF] |> parseUtf8 == Err InvalidUtf8

## Minimum valid 2-byte character
expect [0xC2, 0x80] |> parseUtf8 == Ok { codePt: fromU32Unchecked 0x0080, bytesParsed: 2 }

## Maximum valid 2-byte character
expect [0xDF, 0xBF] |> parseUtf8 == Ok { codePt: fromU32Unchecked 0x07FF, bytesParsed: 2 }

## Minimum valid 3-byte character
expect [0xE0, 0xA0, 0x80] |> parseUtf8 == Ok { codePt: fromU32Unchecked 0x0800, bytesParsed: 3 }

## Maximum valid 3-byte character
expect [0xEF, 0xBF, 0xBF] |> parseUtf8 == Ok { codePt: fromU32Unchecked 0xFFFF, bytesParsed: 3 }

## Minimum valid 4-byte character
expect [0xF0, 0x90, 0x80, 0x80] |> parseUtf8 == Ok { codePt: fromU32Unchecked 0x10000, bytesParsed: 4 }

## Maximum valid 4-byte character
expect [0xF4, 0x8F, 0xBF, 0xBF] |> parseUtf8 == Ok { codePt: fromU32Unchecked 0x10FFFF, bytesParsed: 4 }
