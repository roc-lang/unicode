...
BidiProperties :: [].{
		...
		match InternalBidiProperties.mirroring_glyph(Scalar.to_u32(scalar)) {
			...
			Some(target) => {
				...
				mapped = match Scalar.from_u32(target) {
					Ok(value) => value
					Err(_) => {
						crash "generated bidi mirror target is not a Unicode scalar"
					}
				}
				Some(mapped)
			}
		...
		match InternalBidiProperties.paired_bracket(Scalar.to_u32(scalar)) {
			...
			Some(pair) => {
				...
				mapped = match Scalar.from_u32(pair.scalar) {
					Ok(value) => value
					Err(_) => {
						crash "generated bidi bracket target is not a Unicode scalar"
					}
				}
	...
	paired_bracket_type_short = |kind| match kind {
		None => "n"
		Open => "o"
		Close => "c"
	}
	...
	paired_bracket_type_long = |kind| match kind {
		None => "None"
		Open => "Open"
		Close => "Close"
	}
