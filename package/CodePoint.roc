...
is_valid_scalar = |code_point| !(is_high_surrogate(code_point) or is_low_surrogate(code_point))
...
parse3 = |first, second, third|
    if is_continuation_byte(second) and is_continuation_byte(third) then
        answer =
            first
        ...
        if answer < 0x80 then
            Err(OverlongEncoding)
        else if 0xd800 <= answer and answer <= 0xdfff then
...
parse4 = |first, second, third, fourth|
    if is_continuation_byte(second) and is_continuation_byte(third) and is_continuation_byte(third) then
        answer =
            first
            |> Num.bitwise_and(0b00001111)
            |> Num.to_u32
...
parse_partial_utf8 = |bytes|
    ...
    if first_byte <= 0b0111_1111 then
    ...
    else if first_byte >= 0b1100_0000 and first_byte <= 0b1101_1111 then
        # 2-byte code point
        second_byte = byte_at(1)?
        bytes_parsed = 2

        parse2(first_byte, second_byte)
        |> Result.map_ok(|u32| { code_point: from_u32_unchecked(u32), bytes_parsed })
    else if first_byte >= 0b1110_0000 and first_byte <= 0b1110_1111 then
        # 3-byte code point
        second_byte = byte_at(1)?
        ...
        if Num.bitwise_and(first_byte, 0b11110000) == 0b11100000 then
            ...
            Err(InvalidUtf8)
    else if first_byte >= 0b1111_0000 and first_byte <= 0b1111_0111 then
        # 4-byte code point
        second_byte = byte_at(1)?
        third_byte = byte_at(2)?
        ...
        if
            ...
            == 0b11110000
            and Num.bitwise_and(second_byte, 0b11000000)
            == 0b10000000
            and Num.bitwise_and(third_byte, 0b11000000)
            == 0b10000000
            and Num.bitwise_and(fourth_byte, 0b11000000)
            == 0b10000000
        then
