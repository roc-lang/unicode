...
# TODO: replace with CodePoint :: InternalCP.{...} when possible.
## A [Unicode code point](http://www.unicode.org/glossary/#code_point).
CodePoint :: { cp: InternalCP }.{
    ...
    from_u32 = |u32| {
        if u32 <= 0x10FFFF {
            Ok({cp: InternalCP.from_u32_unchecked(u32)})
        } else {
    ...
    parse_partial_utf8 = |bytes| {
        ...
        if first_byte <= 0b0111_1111 {
            ...
            Try.Ok({code_point: u32_to_cp_unchecked(first_byte.to_u32()), bytes_parsed})
            ...
            .map_ok(|u32| { code_point: u32_to_cp_unchecked(u32), bytes_parsed })
...
u32_to_cp_unchecked = |safe_u32| {
    CodePoint.from_u32(safe_u32) ?? { crash "Unreachable: the u32 is known to be a valid codepoint."}
}
