...
template = {
    ...
    \\    GBP : [${prop_strs}]
    \\
    \\    from_cp : InternalCP -> GBP
    \\    from_cp = |cp| {
    \\        u32 = InternalCP.to_u32(cp)
    \\
    \\        ${is_xtemp(list_meta, "")}
    \\    }
    \\
...
cps_for_property = |current| {
    Helpers.filter_property_map(
        file_map,
        |{ cp, prop }| { if prop == current { Ok(cp) } else { Err(NotNeeded) } }
    )
}
...
unicode_hex_to_test = |(hex, gbp_expected)| {
    ...
    \\expect {
    \\    gbp = InternalGBP.from_cp(InternalCP.from_u32_unchecked(${u32.to_str()}))
    \\    gbp == ${gbp_expected}
    \\}
}
