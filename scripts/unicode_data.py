...
def loose_alias_hash(value: str) -> int:
    ...
    result = 2166136261
    for byte in loose_alias(value).encode("utf-8"):
        result = ((result ^ byte) * 16777619) & 0xFFFFFFFF
    return result


def _packed_loose_alias(value: str) -> tuple[int, int, int, int]:
    normalized = loose_alias(value).encode("utf-8")
    if len(normalized) > 24:
        raise DataError(f"loose Script alias exceeds packed matcher width: {value!r}")
    padded = normalized + bytes(24 - len(normalized))
    words = tuple(
        int.from_bytes(padded[index : index + 8], "little")
        for index in range(0, 24, 8)
    )
    return (words[0], words[1], words[2], len(normalized))
...
def render_script_data(
    ...
    def alias_condition(alias: str, identity: str) -> str:
        word0, word1, word2, length = _packed_loose_alias(alias)
        return (
            f"if loose_eq(value, 0x{word0:016X}, 0x{word1:016X}, "
            f"0x{word2:016X}, {length}) Some({identity})"
        )

    for hash_value, bucket in sorted(from_alias_buckets.items()):
        conditions = " else ".join(
            alias_condition(alias, identity)
